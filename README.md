# 🏨 Hostaway Client SDK for PHP

A modern PHP SDK for the [Hostaway API]([[https://api.hostaway.com/](https://api.hostaway.com/documentation)](Hostaway), designed to simplify integration with Hostaway’s property management system — including reservations, listings, messages, guests, and more.

---

## 🚀 Features

- Clean and fluent API for Hostaway endpoints  
- Built-in authentication with API Key  
- Support for GET, POST, PUT, DELETE requests  
- Laravel compatible (can be used as a package)  
- Easy to extend for new Hostaway API endpoints  
- Fully tested with PEST

---

## 📦 Installation

Install the package via Composer:

```bash
composer require rana-tuhin/hostaway-client-sdk
